      ...
      <div className="mx-auto max-w-md">
          ...
          <div className="space-y-4">
            <div className="rounded-3xl bg-white p-5 shadow-sm">
              <div className="mb-4 flex items-start justify-between gap-3">
                <div>
                  <div className="text-xl font-bold">병원별 KPI</div>
                  <div className="mt-1 text-sm text-gray-500">
                    병원별 제품 수량을 입력하면 revenue와 지분율을 계산합니다.
                  </div>
                </div>

                <button
                  type="button"
                  onClick={addKpiHospital}
                  className="shrink-0 rounded-2xl bg-blue-600 px-4 py-2 text-sm font-bold text-white"
                >
                  병원 추가
                </button>
              </div>
              ...
              <button
                type="button"
                onClick={applyHospitalTotalsToKpi}
                className="w-full rounded-2xl bg-gray-200 p-3 text-sm font-bold"
              >
                병원별 합계를 KPI Actual에 반영
              </button>
            </div>

            {kpiData.hospitals.map((hospital) => {
              const hospitalResult = kpiSummary.hospitalResults.find(
                (result) => result.id === hospital.id,
              );
              const share = kpiSummary.hospitalTotalRev
                ? ((hospitalResult?.rev ?? 0) / kpiSummary.hospitalTotalRev) *
                  100
                : 0;
                ...
                >
                  <div className="mb-4 flex items-center gap-2">
                    <input
                      value={hospital.name}
                      onChange={(e) =>
                        updateKpiHospitalName(hospital.id, e.target.value)
                      }
                      placeholder="병원명"
                    ...
                    <button
                      type="button"
                      onClick={() => removeKpiHospital(hospital.id)}
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            type="button"
                            onClick={() =>
                              changeKpiHospitalProduct(
                                hospital.id,
                                product.name,
                                -1,
                              )
                            }
                            className="rounded-xl bg-gray-200 px-3 py-2 font-bold"
                          >
                            -
                          </button>
                          ...
                          <input
                            type="number"
                            value={hospital.products[product.name]}
                            onChange={(e) =>
                              updateKpiHospitalProduct(
                                hospital.id,
                                product.name,
                                Number(e.target.value),
                              )
                  ...
                  <div className="mt-4 rounded-2xl bg-gray-50 p-4 text-sm">
                    ...
                    <div className="mt-2 text-gray-700">
                      ${formatCurrency(hospitalResult?.rev ?? 0)} / ₩
                      {formatCurrency((hospitalResult?.rev ?? 0) * kpiData.rate)}
                    </div>
                  </div>
                </div>
              );
            ...
            {kpiData.hospitals.length === 0 && (
              <div className="rounded-3xl bg-white p-5 text-sm text-gray-500 shadow-sm">
                병원을 추가해서 제품별 수량을 입력하세요. 병원 tracking이
                어려운 물량은 병원명을 미배정으로 만들어 넣으면 됩니다.
              </div>
            )}

            <div className="rounded-3xl bg-white p-5 shadow-sm">
              <div className="mb-4 text-lg font-bold">제품별 병원 합계</div>

              <div className="space-y-3">
                {kpiProducts.map((product) => {
                  const actual = kpiSummary.hospitalProductTotals[product.name];
                  const asp = kpiData.products[product.name].asp;
                  const rev = actual * asp;
                  ...
                  return (
                    <div key={product.name}>
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <span>
                          {actual}개 / ${formatCurrency(rev)}
                        </span>
                      </div>

                      <div className="mt-1 text-xs text-gray-500">
                        KPI 탭 Actual: {kpiData.products[product.name].actual}개
                      </div>
                    </div>
                  );
                })}
            ...
            <div className="rounded-3xl bg-white p-5 shadow-sm">
              ...
              <div className="space-y-3">
                {Object.entries(kpiSummary.hospitalGroups).map(
                  ([name, group]) => (
                    <div key={name} className="rounded-2xl bg-gray-50 p-4">
                      <div className="mb-2 text-lg font-bold">{name}</div>
            ...
            <div className="rounded-3xl bg-white p-5 shadow-sm">
              <div className="mb-4 text-xl font-bold">병원별 TOTAL KPI</div>

              <div className="rounded-2xl bg-gray-50 p-4 text-lg font-bold">
                연:{" "}
                <span className={achievementClass(kpiSummary.hospitalTotalAch)}>
                  {kpiSummary.hospitalTotalAch.toFixed(1)}%
                </span>{" "}
                | 월:{" "}
                <span
                  className={achievementClass(kpiSummary.hospitalMonthlyAch)}
                >
                  {kpiSummary.hospitalMonthlyAch.toFixed(1)}%
                </span>
              </div>
              ...
              <div className="mt-4 space-y-1 text-sm">
                <div>목표: ${formatCurrency(kpiSummary.totalTarget)}</div>
                <div>
                  병원별 누적: ${formatCurrency(kpiSummary.hospitalTotalRev)}
                </div>
                <div>
                  KPI 탭 누적: ${formatCurrency(kpiSummary.totalRev)}
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 border-t bg-white">
          <div className="mx-auto flex max-w-md">
            ...
            <button
              onClick={() => setTab("hospitalKpi")}
              className={`flex-1 p-4 font-bold ${
                tab === "hospitalKpi" ? "text-blue-600" : "text-gray-500"
              }`}
            >
              병원KPI
            </button>
